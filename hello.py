...
with tab_rank:
    if t_league and not df_scores.empty:
        ...
        now_jst = get_jst_now()
        if period == "今日":
            df_f = df_l[df_l["日付"].dt.date == now_jst.date()]
        elif period == "今月":
            df_f = df_l[(df_l["日付"].dt.year == now_jst.year) & (df_l["日付"].dt.month == now_jst.month)]
        elif period == "前月":
        ...
        if not df_f.empty:
            ...
            for i, row in rank_df.iterrows():
                ...
                if c_n.button(name, key=f"user_{name}"):
                    st.session_state.detail_p = name
                    st.rerun()
...
with tab_setting:
    ...
    with m3:
        if not df_scores.empty:
            for i, r in df_scores.iloc[::-1].head(15).iterrows():
                with st.container(border=True):
