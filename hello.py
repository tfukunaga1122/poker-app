...
# --- 【高速化】データ読み込みをキャッシュ化 ---
@st.cache_data(ttl=300) # 5分間はGoogleシートに見に行かず、メモリから読み込む
def load_all_data():
    try:
        # 必要なシートをまとめて取得
...
with tab_rank:
    if t_league and not df_scores.empty:
        ...
        if not df_f.empty:
            ...
            # 殿堂入り
            st.markdown(f'''<div class="hall-of-fame"><div style="font-size:0.7em; color:#d4af37; font-weight:bold; margin-bottom:5px;">🏆 レコード</div>
...
with tab_input:
    if t_league and not df_players.empty:
        ...
        check_msg = "OK" if total_in == 0 else f"{total_in:+,}"
        st.write(f"収支チェック: {check_msg}")
        ...
        if cs.button("🚀 保存"):
            try:
                with st.spinner('保存中...'):
                    new_scores = pd.concat([df_scores, pd.DataFrame(entries)], ignore_index=True)
                    conn.update(spreadsheet=url, worksheet="scores", data=new_scores)
                    st.cache_data.clear() # 保存後はキャッシュを消して最新にする
                    st.session_state.input_rows = 1
